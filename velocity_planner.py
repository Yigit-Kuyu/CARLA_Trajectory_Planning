...
class VelocityPlanner:
    ...
    ## Son planlanan cycle'dan bu yana geçen timestamp ve önceki planlanan trajectory'ye dayanarak,
    ## open loop speed'i  tahmin eder.
    def get_open_loop_speed(self, timestep):
        ...
        ## Eğer simulation time step=0, trajectory'nin başlangıcını open loop estimate olarak verin.
        if timestep < 1e-4:
            return self._prev_trajectory[0][2]
    ...
    ## Bir yol izler ve istediğimiz hıza ulaşmamız için bir hız profili hesaplar.
    ## - decelerate_to_stop durma çizgisine yavaşlamamız gerekip gerekmediğini belirtir.
    ## - follow_lead_vehicle, durumu lead_car_state'e sahip bir lider aracı takip etmemiz gerekip gerekmediğini belirtir.
    ## Bu durumların ele alınmasında öncelik sırası, dur işaretinin ele alınması, 
    ## öndeki aracın ele alınması ve ardından nominal şerit bakımıdır. Gerçek bir hız planlayıcısında, 
    ## bu durumlar arasındaki eşleşmeyi halletmeniz gerekir, ancak basit olması için bu proje, her durumu izole ederek uygulanabilir.
    ...
    # In the context of the provided code, a trapezoidal profile refers to a velocity profile 
    # that resembles a trapezoid shape. It is a commonly used velocity profile for motion planning and control.
    #A trapezoidal profile is characterized by distinct phases: acceleration, constant velocity, and deceleration. 

    ## Durmak için yavaşlamak için bir trapezoidal profile hesaplar.
    ## Sağlanan kodda, a trapezoidal profile, yamuk şekline benzeyen bir hız profilini ifade eder.  
    ## Hareket planlama ve kontrol için yaygın olarak kullanılan bir hız profilidir.
    ## A trapezoidal profile farklı aşamalarla karakterize edilir: hızlanma, sabit hız ve yavaşlama.
    ...
    def decelerate_profile(self, path, start_speed): 
        ...
        ## Fren mesafesi yolun uzunluğunu aşarsa, yumuşak bir yavaşlama yapamayız 
        ## ve daha sert bir yavaşlama gerekir.
        ## Gerekli zamanda sıfır hıza ulaştığımızdan emin olmak için yolu ters yönde oluşturun.
        ...
        ## Aksi durumda, a full trapezoidal profile yapacağız.
        ## Brake_index, frenlemeye başladığımız yolun indeksi olacak ve 
        ## decel_index, slow_speed'i azaltmsyı bıraktığımız indeks olacak.
        ## Bu iki indeks,trapezoidal profile'deki ramp'ların bitiş noktalarını göstermektedir.
        else:
            ...
            ## slow_speed'e yavaşlamayı durduracak indeksi hesaplayın. 
            ## Bu, yolun başlangıcından başlayarak, söz konusu dizine mesafenin decel_distance'ı toplanana kadar noktalar arasında adım atılarak yapılır.   
            decel_index = 0
            temp_dist = 0.0
            ...
            ## Başlangıçtan decel_index'e olan hızlar, mevcut hızdan slow_speed'e dogru, dogrusal rampa olmalıdır ve
            ## -self._a_max'ta yavaşlamalıdır.
            vi = start_speed
    ...
    ## Lead vehicle'ı takip etmek için profil hesapla.
    def follow_profile(self, path, start_speed, desired_speed, lead_car_state):
        ...
        ## Planed path'de öndeki araca en yakın noktadan hızımızın istenen hızın minimumunda 
        ## ve ego aracın hızında sabit tutulduğunu varsayarak zaman aralığı noktasını hesaplayın.
        ...
        ## Zaman aralığı noktasına ulaştığımızda, istenen hızda olmamız gerekir. 
        ## Oraya a_max kullanarak ulaşamıyorsak, daha hızlı yavaşlamak için denetleyiciyi kullanmak üzere profilde ani bir değişiklik yapın.
