...
class LocalPlanner:
    ...
    ## Belirli bir hedef konumundan goal state'i hesaplar. 
    ## Bu, ego aracının hedef sapmasına dik yön boyunca, hedef location'dan yanal (lateral) olarak örnekleme offsetleri ile yapılır.
    def get_goal_state_set(self, goal_index, goal_state, waypoints, ego_state):
        ...
        ## Bir hedef konumu verilen goal state'leri elde eder
        ...
        ## Final yönü next indexe bağlı olarak hesaplayın.
        ##  goal index waypoint seti içinde sonuncuysa, önceki indeksi kullan.
        ## Bunu kullanmak için, delta_x ve delta_y değerlerini sıralı waypointler arasında hesapla ve
        ## arctan2() fonksiyonunu kullan.
        ...
        ## local frame'deki center goal state durumunu, ego state'i kullanarak hesaplayın.
        ## Aşağıdaki, kod, input goal state'i ego vehicle'in local frame'ine dönüştürecektir.
        ...
        ## Ego state'i yeni frame'de başlangıç noktasında olacak  şekilde dönüştürün.
        ## Bu, ego state'i goal_state_local'dan çıkarılarak yapılır.
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        ...
        ##  New frame'de ego state sıfır yönü olacak şekilde döndür.
        ##  Genel döndürme matrisinin [cos(theta) -sin(theta) sin(theta) cos(theta)] olduğunu  
        ##  ve  yeni local frame'de ego aracının mevcut sapmasının 
        ##  teta = 0'a karşılık gelmesini sağlamak için -ego_state[2] tarafından döndürüldüğünü hatırlayın.
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        theta = -ego_state[2]
        ...
        ## Yön değişkeninden mevcut ego sapmasını çıkararak yerel frame'deki hedef sapmasını hesaplayın.
        # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
        # ------------------------------------------------------------------
        ...
        ## her path için offset'i hesapla ve uygula, şöyleki
        ## tüm yollar hedef state'in aynı yönüne sahiptir, fakat
        ## hedef istikametine göre yanal (lateral) offset olarak kaydırılır.
        goal_state_set = []
        for i in range(self._num_paths):
            ...
            ## Local planner için ayarlanan path sayısını içeren offset'leri hesapla.
            ## Her offset hedefi, local planner tarafından değerlendirilecek bir potantial path oluşturur.
            ...
            ## x ve y ekseni boyunca yanal (lateral) ötelemenin izdüşümünü hesaplayın.
            ## Bunun için, offset'i sırasıyla cos(goal_theta + pi/2) ve sin(goal_theta + pi/2) ile çarpın.
            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
            # ------------------------------------------------------------------
    ...
    ## Path set'i  her bir goal state'e polynomial spiral optimization kullarak hesapla.
...
def transform_paths(paths, ego_state):
    ...
    for path in paths:
        ...
        for i in range(len(path[0])):
            ## dynamic calculation of the transformation equations
