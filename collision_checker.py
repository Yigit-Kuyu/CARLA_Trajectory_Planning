...
class CollisionChecker:
    ...
    ## Bir dizi yolu ve engeli alır ve her yolun çarpışmasız olup olmadığını 
    ## söyleyen bir bool dizisi döndürür.
    def collision_check(self, paths, obstacles):
        """Returns a bool array on whether each path is collision free.

        ## Yolun carpismasız oldugunu check eden bır bool array dondurur (1/0, True/False)
        ...
        for i in range(len(paths)):
            ...
            ## Yoldaki noktalar boyunca iterasyon
            for j in range(len(path[0])):
                ...
                ## Yoldaki noktalar boyunca daire konumunu hesaplayın. Bu daireler,
                ## engellere sahip her bir path boyunca olası çarpışmalara kontrol etmek için
                ## aracın yaklaşık çarpışma sınırını belirler.

                ## Daire ofset'leri "self._circle_offsets" tarafından verilir. Daire offsetleri
                ## path boyunca her bir noktaya yerleştirilmesi (iterasyon boyunca) gerekir.
                ## Her path [[x_values], [y_values], [theta_values]] biçimindedir.
                
                ## Bu nedenle, şunu hesaplamamız gerekir:
                ## circle_x = point_x + circle_offset*cos(yaw)
                ## circle_y = point_y circle_offset*sin(yaw)
                ## for each point along the path.
                ## point_x is given by path[0][j], and point _y is given by
                ...
                # the collision_free flag should be set to false for this 
                
                ## Her bir engelin, [x, y] biçimindeki noktaların bir koleksiyonuyla yaklaştığını varsayar.
                ## Burada engel noktalarını iterasyon edeceğiz ve engel noktalarının dairemizin herhangi birinin içinde mi diye check edeceğiz.
                ## Eğer içindeyse, collision_free=false olur.
    ...
    ## Şerit merkez çizgisini (lane centerline) ne kadar yakından takip ettiğine ve çarpışma halindeki 
    ## diğer path'lerden ne kadar uzakta olduğuna bağlı olarak yol grubundaki en iyi yolu seçer.
    ## Engellerle çarpışan yolları seçim sürecinden diskalifiye eder.
    ## collision_check_array, True'yu içerir eğer paths[i] çarpışmasız ise, diğer durumda False'dur.
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        ## Aracın geçmesi için en uygun yol indeksini verir.
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                ## "Merkez çizgisinden uzaklık" puanını hesaplayın. Merkez çizgisi hedefi, goal_state tarafından verilir.
                ## Amaç fonksiyonunun kesin seçimi size kalmış. Daha düşük bir puan, daha uygun bir yol anlamına gelir.
                # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                ...
                ## "Diğer çarpışan yollara yakınlık" puanını hesaplayın ve
                ## "merkez çizgisinden uzaklık" puanına ekleyin.
                ## Amaç fonksiyonunun kesin seçimi size kalmış.
